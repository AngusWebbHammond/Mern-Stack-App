    ...
    >
      ...
      <div className="overflow-y-auto">
        <div className="flex flex-col bg-gray-200 dark:bg-slate-700 p-3 gap-3">
          {props.data
            ?.filter((item) => item.type === props.todoListDict._id)
            .map((todoItemDict: TodoType) => (
              <TodoItem
                isTitleUpdating={props.isTitleUpdating}
                setIsTitleUpdating={props.setIsTitleUpdating}
                index={props.data?.findIndex(
                  (item) => item._id === todoItemDict._id
                )}
      ...
      <div className="p-3 relative">
        <AddNewTodo
          id={props.todoListDict._id}
          todoTitle={props.todoListDict.title}
          h3TextStyling={props.h3TextStyling}
