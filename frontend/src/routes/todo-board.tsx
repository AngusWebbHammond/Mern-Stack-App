...
function TodoComponent(props: Props) {
  ...
  // Variables
  const h1TextStyling: string =
    "text-black dark:text-white font-medium text-2xl flex justify-left";
  const h3TextStyling: string =
    "text-gray-800 dark:text-gray-100 font-bold text-md flex justify-left items-center";
  const h4TextStyling: string =
    "text-gray-700 dark:text-gray-300 font-normal text-sm flex justify-left";
  ...
  function addNewTodo(typeID: string, type: string): void {
    ...
    fetch(addNewTodoURL, { method: "POST" }).then(() => {
      setIsDataUpdating(!isDataUpdating);
      return;
    });
  }
  ...
  function addNewTodoList(): void {
    ...
    fetch(addNewTodoListURL, { method: "POST" }).then(() => {
      setIsDataUpdating(!isDataUpdating);
    });
  }
  ...
  return (
    <div
      className={`flex flex-row gap-2 justify-center h-full px-20 bg-white dark:bg-slate-900 
    ...
    >
      {props.type === "Board" ? (
        ...
        <TodoBoardList data={data} todoLists={todoLists} />
      ) : (
